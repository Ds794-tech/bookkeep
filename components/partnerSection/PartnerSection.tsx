...
export default function PartnerSlider() {
  ...
  return (
    <section className="py-5 md:py-20 md:bg-gray-50">
      <div className="max-w-7xl mx-auto px-6">
        <Slider {...settings}>
          {logos.map((logo, idx) => (
            <div key={idx} className="px-4 py-4 md:py-12">
              <div className="bg-white shadow-md rounded-lg flex items-center justify-center md:p-4 h-24">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={logo}
                  alt={`Partner ${idx + 1}`}
                  className="h-12 w-full object-contain grayscale hover:grayscale-0 transition duration-300"
                />
              </div>
            </div>
          ))}
